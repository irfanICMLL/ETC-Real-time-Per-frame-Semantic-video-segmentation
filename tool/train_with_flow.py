...
from util import transform_video as transform
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/cityscapes/cityscapes_pspnet18_flow.yaml', help='config file')
...
def check(args):
    ...
    if args.arch == 'psp' or args.arch == 'psp18':
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, frames, target) in enumerate(train_loader):
        ...
        res = model(input, frames, target, 'train')
        if not args.multiprocessing_distributed:
            main_loss, aux_loss = torch.mean(res['ce_loss']), torch.mean(res['st_loss'])
        else:

            main_loss = res['ce_loss']
            aux_loss = res['st_loss']
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, _,target) in enumerate(val_loader):
        ...
        input = input.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        output = model(input, None,None, 'test')
        if args.zoom_factor != 8:

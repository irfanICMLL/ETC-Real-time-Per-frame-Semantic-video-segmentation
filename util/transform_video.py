...
class Compose(object):
    ...
    def __call__(self, image, frame, label):
        for t in self.segtransform:
            image, frame, label = t(image, frame, label)
...
class ToTensor(object):
    ...
    def __call__(self, image, frame, label):
        ...
        if not isinstance(frame, torch.FloatTensor):
            frame = frame.float()
    ...
    def __call__(self, image, frame, label):
        if self.std is None:
            ...
            for t, m, s in zip(image, self.mean, self.std):
                t.sub_(m).div_(s)
        if self.std is None:
            for t, m in zip(frame, self.mean):
                t.sub_(m)
        else:
...
class Resize(object):
    ...
    def __call__(self, image, frame, label):
        ...
        frame = cv2.resize(frame, self.size[::-1], interpolation=cv2.INTER_LINEAR)
...
class RandScale(object):
    ...
    def __call__(self, image, frame, label):
        ...
        frame = cv2.resize(frame, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_LINEAR)
...
class Crop(object):
    ...
    def __call__(self, image, frame, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            frame = cv2.copyMakeBorder(frame, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half,
                                       cv2.BORDER_CONSTANT, value=self.padding)
        ...
        frame = frame[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w]
...
class RandRotate(object):
    ...
    def __call__(self, image, frame, label):
        if random.random() < self.p:
            ...
            frame = cv2.warpAffine(frame, matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                                   borderValue=self.padding)
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, image, frame, label):
        if random.random() < 0.5:
            ...
            frame = cv2.GaussianBlur(frame, (self.radius, self.radius), 0)
        return image, frame, label
